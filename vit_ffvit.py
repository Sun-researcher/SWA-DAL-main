...
class Attention(nn.Module): 
    ...
    def forward(self, x):
        # [batch_size, num_patches + 1, total_embed_dim] #num_patches + 1 class token
        B, N, C = x.shape
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        y = self.patch_embed(x)  # [B, 196, 768]
        # [1, 1, 768] -> [B, 1, 768]
        cls_token = self.cls_token.expand(y.shape[0], -1, -1)
            ...
            if self.training and self.ssrt:
                if layer == perturb_layer: 
                    ...
                    if np.random.rand() > 0.5:
                        ...
                        ym = y[B // 2:] + (y[idx]-y[B // 2:]).detach() * (1-self.sr_alpha_adap)
                    y = torch.cat((y, ym))
                y = blk(y)
            else:
                y = blk(y)
...
if __name__ == '__main__':
    model = vit_base_patch16_224_in21k(num_classes=2)
